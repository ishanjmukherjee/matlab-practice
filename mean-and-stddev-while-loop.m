...
% Receives input as long as input is non-numeric.
fprintf("Enter anything non-numeric to terminate input process\n")
while isnumeric(x)
    n = n + 1;
    % Adding sum(inputs) to sum variable in case user inputs a numeric 
    % array.
    sum_x = sum_x + sum(x);
...
fprintf("The standard deviation is %f", std_dev);
